...
| `FDBcc` | No coprocessor DBcc translator. `INSN(fscc, f240, ffc0, FPU)` matches `f248` (F-line + register in the low bits), so FDBcc is decoded as FScc. | MC68020 UM 7.2.2.3.2 / 7.4.1: cpDBcc decrements Dn and branches; scanPC for the displacement is the 16-bit displacement word (instruction+4). | QEMU `translate.c` `INSN(fscc, f240, ffc0)`. Do not change QEMU. |
| EMAC_B dual MAC (`MAAAC`/`MASAC`/`MSAAC`/`MSSAC`) | `DISAS_INSN(mac)` sets `dual` only when `(insn & 0x30) != 0 && (ext & 3) != 0`, so register-only dual encodings such as `a0010005` are a single MAC into one accumulator. | CFPRM EMAC_B: register-only `MAAAC.sz Ry,RxSF,ACCx,ACCw` adds the scaled product to both accumulators. N/Z/V/EV follow the secondary result; PAVx/PAVw record overflow per destination. | QEMU `translate.c` `dual = ((insn & 0x30) != 0 && (ext & 3) != 0)`. Do not change QEMU. |
